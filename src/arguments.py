...
class TrainingArguments(TrainingArguments):
    ...
    kd_loss_type: str = field(default="ce_only", metadata={"help": "kd criterion: one of {ce_only, default, default_distillation, emkd, em_kd, sre, joint, unit_aligned, scva, cgkd, scva_cgkd, ugrimd, draft, mcw_kd, dskd_v2_with_eta}"})
    # UGRIMD (uncertainty-gated ranking + interaction multimodal distillation)
    distill_lambda: float = field(default=1.0, metadata={"help": "UGRIMD weight for ranking plus interaction distillation."})
    interaction_lambda: float = field(default=1.0, metadata={"help": "UGRIMD weight for the interaction loss within distillation."})
    num_candidates: int = field(default=8, metadata={"help": "UGRIMD candidates retained per sample, including the ground truth."})
    candidate_regenerate_steps: int = field(default=500, metadata={"help": "UGRIMD candidate-cache regeneration interval in optimizer steps."})
    uncertainty_top_percent: float = field(default=30.0, metadata={"help": "UGRIMD top-percent gate for useful within-sample rank pairs."})
    hard_sample_top_percent: float = field(default=30.0, metadata={"help": "UGRIMD top-percent student-uncertain samples selected for stage-2 teacher supervision."})
    hard_sample_refresh_steps: int = field(default=1000, metadata={"help": "UGRIMD optimizer-step interval for refreshing hard-sample selection."})
    exploration_percent: float = field(default=10.0, metadata={"help": "UGRIMD random exploration percentage of non-selected interaction samples."})
    rank_temperature: float = field(default=0.1, metadata={"help": "UGRIMD pair-preference sigmoid temperature."})
    rank_epsilon: float = field(default=1e-8, metadata={"help": "UGRIMD ranking numerical epsilon."})
    visual_attention_layers: List[int] = field(default_factory=lambda: [1, 2, 3], metadata={"help": "UGRIMD early teacher decoder-attention layers used for ranking reliability."})
    visual_weight_temperature: float = field(default=0.1, metadata={"help": "UGRIMD vision-aware ranking agreement temperature."})
    visual_weight_floor: float = field(default=0.25, metadata={"help": "UGRIMD minimum vision-aware ranking reliability weight."})
    candidate_temperature: float = field(default=1.0, metadata={"help": "UGRIMD candidate-score distribution temperature."})
    interaction_temperature: float = field(default=1.0, metadata={"help": "UGRIMD SmoothL1 beta for multimodal interaction matching."})
    interaction_strength_temperature: float = field(default=0.1, metadata={"help": "UGRIMD teacher visual-shift gate temperature."})
    loss_epsilon: float = field(default=1e-8, metadata={"help": "UGRIMD loss normalization epsilon."})
